...
import { loginInstitute } from "../api/auth.api";
...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await loginInstitute(formData);
    ...
    <div className="w-full max-w-[420px] p-8 sm:p-10 rounded-[32px] bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
      <div className="mb-8 text-center">
        ...
        <p className="text-sm text-gray-400">
          Sign in to your institute's workspace
        </p>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-white/10 text-center">
        <p className="text-sm text-gray-400">
          Don't have an institute workspace yet?{" "}
